...
var acchk = window.localStorage.getItem("codestatus");	
//alert(acchk);
if ((user.access_code != "" || user.access_code) && acchk == "valid") {

	if (window.localStorage.getItem("Accessto") == "custom") {
	...
	} else if (window.localStorage.getItem("Accessto") != "all") {
		var accessto = window.localStorage.getItem("Accessto");
		var ACCCAT = accessto.split(',');
	}
		...
		if (childSnapshot.key == ct) {
			...
			$.map(childData.Session, function (value, index) {
				...
				if ((loc - 1 >= temp && childSnapshot.key != "Open Dive")) {
					...
					if ((temp == 0 && $.inArray(value.session_id, Shistory) != -1) || (temp != 0 && $.inArray(value.session_id, Shistory) != -1 && ($.inArray(ct,ACCCAT) != -1 || $.inArray(value.session_id,ACCSess) != -1 || accessto == "all")) ) {
						//alert(temp);
						ht = ht + '<div class="col-md-4 col-xs-6 boxStyle position-relative p-0 " style=" background-image: url(' + value.session_img + '); "><p class="Center bundle" data-count ="' + count + '"data-cat="' + cid + '" id="' + value.session_id + '">' + value.session_name + '</p><div class="box1"><i class="fa fa-check-circle center"></i></div></div>';
					}
					else if (temp != 0 && ( $.inArray(ct,ACCCAT) == -1 && $.inArray(value.session_id,ACCSess) == -1 && accessto != "all")) {
						//alert("temp");
					...
					else if (($.inArray(value.session_id,Shistory) == -1 && temp == 0) || (temp != 0 && $.inArray(ct,ACCCAT) != -1) || (temp != 0 && $.inArray(value.session_id,ACCSess) != -1) || (temp != 0 && accessto == "all") ) {
						ht = ht + '<div class="col-md-4 col-xs-6 boxStyle position-relative p-0 " style=" background-image: url(' + value.session_img + '); "><p class="Center bundle" data-count ="' + count + '" data-cat="' + cid + '" id="' + value.session_id + '">' + value.session_name + '</p></div>';
					}
